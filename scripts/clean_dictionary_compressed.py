...
def process_batch(batch: list) -> list:
    ...
    # Allow enough retries to cycle through all keys multiple times if needed
    ...
    for attempt in range(retries):
        try:
            response = client.models.generate_content(
                ...
                config=types.GenerateContentConfig(
                    system_instruction=(
                        ...
                        "Your task is to take a raw list of pipe-delimited Arabic dictionary entries (id|original|romanized|definition) and clean them. "
                        "Rules:\n"
                        "1. Simplify Phrases: If 'original' is a sentence or phrase, reduce it to the single core Arabic word that matches 'definition'.\n"
                        "2. Fix Transliterations: Ensure 'romanized' reflects the standard modern English phonetic pronunciation (e.g., 'sh' instead of 'š', 'kh' instead of 'x', accurate sun/moon letter assimilation like 'ash-shams' vs 'al-qamar').\n"
                ...
                if len(parts) >= 4:
                    ...
                    if wid in batch_dict and wid not in seen_ids:
                        ...
                        cleaned_items.append({
                            ...
                            "language": orig_item.get("language", "ar"),
            ...
            if attempt < retries - 1:
                time.sleep(3)
                # Rotate key on error (e.g. 429 quota / resource exhausted)
                get_next_client()
            else:
...
def main():
    ...
    batch_size = 200 # Conservative batch size to guarantee 100% full recall without lazy AI skipping
